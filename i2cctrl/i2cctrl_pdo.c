...
case IRP_MN_START_DEVICE:
{
    ...
    /* --------------------------------------------------------
       SYNTHETIC PDO PATH (controller hardware failed)
       -------------------------------------------------------- */
    if (fdoExt->HardwareFailure) {

        I2cCtrl_Log(
            "PDO: START_DEVICE - ParentFdo->HardwareFailure=TRUE, "
            "skipping HID I2C access and faking success\n"
        );

        /* Minimal XP-compatible HID descriptor */
        RtlZeroMemory(&pdoExt->HidDesc, sizeof(pdoExt->HidDesc));

        pdoExt->HidDesc.bLength         = sizeof(HID_DESCRIPTOR);
        pdoExt->HidDesc.bDescriptorType = 0x21;   /* HID descriptor */
        pdoExt->HidDesc.bcdHID          = 0x0111; /* HID v1.11 */
        pdoExt->HidDesc.bCountry        = 0;
        pdoExt->HidDesc.bNumDescriptors = 1;

        pdoExt->HidDesc.DescriptorList[0].bReportType  = 0x22; /* Report descriptor */
        pdoExt->HidDesc.DescriptorList[0].wReportLength = 0;   /* no report */

        pdoExt->HidReportDesc     = NULL;
        pdoExt->HidReportDescLen  = 0;
        pdoExt->PendingHidReadIrp = NULL;

        pdoExt->Started = TRUE;

        I2cCtrl_Log(
            "PDO: START_DEVICE synthetic-success for PDO %p (XP HID descriptor)\n",
            DeviceObject
        );
    ...
    /* --------------------------------------------------------
       REAL ACPI HID DEVICE PATH (controller working)
       -------------------------------------------------------- */
