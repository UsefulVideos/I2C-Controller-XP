...
/* -----------------------------------------------------------------------
 * I2cCtrl_PdoDispatchPower - PDO power dispatch
 * XP/2003-safe, WDM-compliant, C89-friendly.
 * ----------------------------------------------------------------------- */
NTSTATUS
...
ps     = IoGetCurrentIrpStackLocation(Irp);
        ...
        ps->Parameters.Power.Type == DevicePowerState)
    {
        ...
        switch (newState) {
        case PowerDeviceD0:
            (void)I2cCtrl_PowerOnTouchpad(DeviceObject);
            pdoExt->CurrentPowerState = PowerDeviceD0;
            pdoExt->Started           = TRUE;
            break;

        case PowerDeviceD3:
            (void)I2cCtrl_PowerOffTouchpad(DeviceObject);
...
case IRP_MN_START_DEVICE:
{
    ...
    /* --------------------------------------------------------
       REAL ACPI HID DEVICE PATH (controller must be working)
       -------------------------------------------------------- */
    ...
    I2cCtrl_Log("PDO: Calling I2cCtrl_PowerOnTouchpad() before HID descriptor read\n");
    (void)I2cCtrl_PowerOnTouchpad(DeviceObject);
...
/* Only start GPIO interrupt thread if ACPI provided a valid GpioInt pin */
if (pdoExt->GpioHandle != NULL &&
    pdoExt->GpioPin   != 0 &&
    pdoExt->GpioValid)
{
    /* -------------------------------------------------------------
     * Start interrupt thread (GPIO-driven HID input)
     * ------------------------------------------------------------- */
    {
        HANDLE threadHandle = NULL;
        NTSTATUS thStatus;

        thStatus = PsCreateSystemThread(
                       &threadHandle,
                       THREAD_ALL_ACCESS,
                       NULL,
                       NULL,
        ...
        if (NT_SUCCESS(thStatus)) {
            ...
            I2cCtrl_Log("PDO: InterruptThread started for %p\n", DeviceObject);
        } else {
            I2cCtrl_Log("PDO: FAILED to start InterruptThread (0x%08X)\n",
                        thStatus);
        }
    }
}
...
{
    I2cCtrl_Log("PDO: No GpioInt resource -> skipping GPIO interrupt thread\n");

    /* -------------------------------------------------------------
     * IOAPIC interrupt (legacy ACPI Interrupt() resource)
     * -------------------------------------------------------------
     * This device uses an IOAPIC GSI (e.g., 0x6D or 0x5D) instead of
     * a GPIO pin. In this mode, the I2C controller's ISR/DPC handles
     * all HID wakeups and input report triggering.
     *
     * Nothing to start here - the FDO ISR is already connected during
     * controller StartDevice and will fire on the ACPI Interrupt().
     */
    I2cCtrl_Log("PDO: Using IOAPIC interrupt -> I2C controller ISR will "
                "handle HID wakeups\n");
}
