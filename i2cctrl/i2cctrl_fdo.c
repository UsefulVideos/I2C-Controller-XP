...
case IRP_MN_QUERY_DEVICE_RELATIONS:
{
    ...
    if (type == BusRelations)
    {
        ...
        {
            //
            // ACPI children always win
            //
            if (!IsListEmpty(&fdoExt->ChildList))
            {
                I2cCtrl_Log("BusRelations: ACPI children present -> returning ACPI list\n");
            }
            else
            {
                //
                // Only attempt HID-over-I2C detection if the controller
                // actually started and did not hit a hardware failure.
                //
                if (fdoExt->Started && !fdoExt->HardwareFailure)
                {
                    I2CCTRL_DETECT_RESULT det;
                    NTSTATUS detStatus;

                    RtlZeroMemory(&det, sizeof(det));

                    I2cCtrl_Log("BusRelations: No ACPI children -> running DetectTouchpad\n");

                    detStatus = g_I2cCtrlGlobal.DetectTouchpad(fdoExt, &det);

                    //
                    // Only create a PDO if:
                    //   - detection succeeded
                    //   - device is present
                    //   - it is a touchpad
                    //   - AND no other controller has claimed it
                    //
                    if (NT_SUCCESS(detStatus) &&
                        det.Present &&
                        det.IsTouchpad &&
                        !g_I2cCtrlGlobal.TouchpadPresent)
                    {
                        I2cCtrl_Log("BusRelations: touchpad detected -> creating synthetic PDO\n");

                        fdoExt->TouchpadPresent = TRUE;
                        g_I2cCtrlGlobal.TouchpadPresent = TRUE;

                        status = I2cCtrl_CreateTouchpadPdo(fdoExt->Self, fdoExt);

                        if (!NT_SUCCESS(status))
                        {
                            I2cCtrl_Log("BusRelations: FAILED to create synthetic PDO (0x%08lx)\n", status);
                        }
                        else
                        {
                            I2cCtrl_Log("BusRelations: synthetic I2CCTRL\\TOUCHPAD PDO created\n");
                        }
                    }
                    else
                    {
                        I2cCtrl_Log(
                            "BusRelations: no touchpad on this controller "
                            "(status=0x%08lx Present=%lu IsTouchpad=%lu GlobalClaim=%lu)\n",
                            detStatus,
                            (ULONG)det.Present,
                            (ULONG)det.IsTouchpad,
                            (ULONG)g_I2cCtrlGlobal.TouchpadPresent
                        );
                    }
                }
                else
                {
                    I2cCtrl_Log(
                        "BusRelations: controller not started or hardware failure -> "
                        "skipping HID detection (Started=%lu HwFail=%lu)\n",
                        (ULONG)fdoExt->Started,
                        (ULONG)fdoExt->HardwareFailure
                    );
                }
            }
        }
