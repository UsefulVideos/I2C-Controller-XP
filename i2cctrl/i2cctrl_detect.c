    ...
    )
{
    ...
    //
    // If controller is NOT started or has hardware failure, do NOT probe the bus.
    //
    if (!dx->Started || dx->HardwareFailure) {
        I2cCtrl_Log(
            "DetectTouchpad: controller not ready (Started=%lu HwFail=%lu) -> skipping detection\n",
            (ULONG)dx->Started,
            (ULONG)dx->HardwareFailure
        );
    ...
    for (i = 0; i < ARRAYSIZE(g_HidI2cCommonCandidates); i++) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            I2cCtrl_Log(
                "DetectTouchpad: HID device found at 0x%02X (VID=0x%04X PID=0x%04X Ver=0x%04X Len=%u)\n",
                addr,
