...
/* -----------------------------------------------------------------------
 * I2cCtrl_DetectTouchpadRedirect - thin wrapper for touchpad detection
 * Validates parameters, zeroes result, and delegates to I2cCtrl_DetectTouchpad.
 * ----------------------------------------------------------------------- */
NTSTATUS
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        I2cCtrl_Log("DetectTouchpadRedirect: detection succeeded (Present=%lu IsTouchpad=%lu)\n",
                    (ULONG)result->Present,
                    (ULONG)result->IsTouchpad);
    }
...
I2cCtrl_ReportPwrmBaseInfo(fdoExt->PwrmBase);

//
// Validate PWRMBASE before doing ANY PMC/LPSS access.
//
status = I2cCtrl_ReportPwrmBaseInfo(fdoExt->PwrmBase);
if (!NT_SUCCESS(status)) {

    I2cCtrl_Log("StartDevice: INVALID PWRMBASE -> controller cannot be powered on\n");
    I2cCtrl_Log("StartDevice: Aborting controller bring-up due to invalid PWRMBASE\n");

    //
    // Mark controller as unsupported, NOT hardware failure.
    // HardwareFailure=TRUE means “controller is broken”.
    // Unsupported=TRUE means “platform does not expose PMC”.
    //
    fdoExt->UnsupportedPlatform = TRUE;
    fdoExt->Started = FALSE;

    //
    // Disconnect interrupt if already connected.
    //
    ...
    //
    // Unmap BAR0 if mapped.
    //
    if (fdoExt->Mmio) {
        I2cCtrl_Log("StartDevice: Unmapping BAR0 MMIO region\n");
        MmUnmapIoSpace(fdoExt->Mmio, fdoExt->MmioLength);
        fdoExt->Mmio = NULL;
        fdoExt->MmioLength = 0;
        fdoExt->MmioPhys.QuadPart = 0;
    ...
    //
    // Return success so PnP stack continues normally,
    // but the controller will never enumerate children.
    //
...
/* -------------------------------------------------------------
 * Map PWRMBASE so we can access WHL power-well registers
 * ------------------------------------------------------------- */
if (fdoExt->PwrmBase.QuadPart != 0 && fdoExt->PwrmBaseVa == NULL)
{
    fdoExt->PwrmBaseVa = MmMapIoSpace(
        fdoExt->PwrmBase,   /* physical FE000000 */
        0x2000,             /* covers PW_CTL/PW_STS/PW_FORCE_ON */
        MmNonCached
    );

    if (fdoExt->PwrmBaseVa != NULL)
        I2cCtrl_Log("StartDevice: PWRMBASE mapped at %p\n", fdoExt->PwrmBaseVa);
    else
        I2cCtrl_Log("StartDevice: MmMapIoSpace for PWRMBASE FAILED\n");
}


/* -------------------------------------------------------------
 * XP fallback: LPSS BAR2 from PWRMBASE + PID offset
 * (Cannon Point-LP / Whiskey Lake)
 * ------------------------------------------------------------- */
if (!haveBar2)
{
    ULONG pidOffset;
    ...
    if (fdoExt->PwrmBase.QuadPart != 0)
    {
        ...
        if (fdoExt->LpssBar2 != NULL) {
            haveBar2 = TRUE;
            I2cCtrl_Log("StartDevice: LPSS BAR2 from PWRMBASE+0x%04lX mapped\n",
                        pidOffset);
        } else {
            I2cCtrl_Log("StartDevice: LPSS BAR2 PCR map FAILED (offset=0x%04lX)\n",
                        pidOffset);
        }
    }
    else
...
/* -------------------------------------------------------------
 * WHL/CNL Power Wells + LPSS Enable (Super Diagnostic Version)
 * ------------------------------------------------------------- */

I2cCtrl_Log("StartDevice: === WHL POWER + LPSS DEBUG BEGIN ===\n");
...
if (fdoExt->PwrmBaseVa != NULL)
{
    ...
    I2cCtrl_Log("StartDevice: PWRMBASE VA = %p\n", fdoExt->PwrmBaseVa);

    /* --- Read initial PMC4 / PW registers --- */
    __try {
        pmc4_before = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PMC_PMC4_OFFSET));
        pw_force_before = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PW_FORCE_ON_OFFSET));
        pw_sts_before = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PW_STS_OFFSET));
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        I2cCtrl_Log("StartDevice: EXCEPTION reading initial PWRM registers\n");
    }
    ...
    /* --- Enable CECE bit --- */
    I2cCtrl_Log("StartDevice: Setting CECE bit (0x%08lx) in PMC4\n", WHL_PMC_CECE_BIT);

    pmc4_after = pmc4_before | WHL_PMC_CECE_BIT;

    __try {
        WRITE_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PMC_PMC4_OFFSET), pmc4_after);
        pmc4_after = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PMC_PMC4_OFFSET));
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        I2cCtrl_Log("StartDevice: EXCEPTION writing/reading PMC4\n");
    }
    ...
    __try {
        WRITE_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PW_FORCE_ON_OFFSET), WHL_PW_MASK);
        pw_force_after = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PW_FORCE_ON_OFFSET));
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        I2cCtrl_Log("StartDevice: EXCEPTION writing/reading PW_FORCE_ON\n");
    }
    ...
    /* --- Poll PW_STS --- */
    {
        ...
        do {
            __try {
                pw = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PW_STS_OFFSET));
            } __except(EXCEPTION_EXECUTE_HANDLER) {
...
{
    I2cCtrl_Log("StartDevice: PWRMBASE VA NULL -> cannot access power wells\n");
}
...
if (fdoExt->LpssBar2 != NULL)
{
    ...
    I2cCtrl_Log("StartDevice: LPSS BAR2 VA = %p\n", fdoExt->LpssBar2);

    /* --- Read initial LPSS state --- */
    __try {
        clk_before = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_CLK_CTL));
        rst_before = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_RST_CTL));
        rst_sts_before = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_RST_STS));
    } __except(EXCEPTION_EXECUTE_HANDLER) {
    ...
    __try {
        WRITE_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_CLK_CTL), 0x7);
        clk_after = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_CLK_CTL));
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        I2cCtrl_Log("StartDevice: EXCEPTION writing/reading LPSS CLK_CTL\n");
    }
    ...
    __try {
        WRITE_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_RST_CTL), 0x0);
        rst_after = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_RST_CTL));
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        I2cCtrl_Log("StartDevice: EXCEPTION writing/reading LPSS RST_CTL\n");
    }
    ...
    /* --- Poll reset status --- */
    {
        ...
        do {
            __try {
                rst = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_RST_STS));
            } __except(EXCEPTION_EXECUTE_HANDLER) {
...
I2cCtrl_Log("StartDevice: === WHL POWER + LPSS DEBUG END ===\n");
...
if (!NT_SUCCESS(status)) {
    ...
    /* --- Extra WHL/CNL power + LPSS debug on ENABLE failure --- */
    I2cCtrl_Log("StartDevice: ENABLE FAILED -> collecting WHL/CNL power + LPSS debug\n");

    /* 1. Dump PWRM / PMC4 / PW_* if mapped */
    if (fdoExt->PwrmBaseVa != NULL) {
        ...
        __try {
            pmc4 = READ_REGISTER_ULONG(
                (PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PMC_PMC4_OFFSET)
            );
            pw_force = READ_REGISTER_ULONG(
                (PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PW_FORCE_ON_OFFSET)
            );
            pw_sts = READ_REGISTER_ULONG(
                (PULONG)((PUCHAR)fdoExt->PwrmBaseVa + WHL_PW_STS_OFFSET)
            );
        } __except(EXCEPTION_EXECUTE_HANDLER) {
            I2cCtrl_Log("StartDevice: EXCEPTION reading PWRM/PMC4/PW_* registers\n");
        }

        I2cCtrl_Log("StartDevice: PWRMBASE VA = %p\n", fdoExt->PwrmBaseVa);
        I2cCtrl_Log("StartDevice: PMC4 @+0x%04X = 0x%08lx\n",
                    WHL_PMC_PMC4_OFFSET, pmc4);
        I2cCtrl_Log("StartDevice: PW_FORCE_ON @+0x%04X = 0x%08lx\n",
                    WHL_PW_FORCE_ON_OFFSET, pw_force);
        I2cCtrl_Log("StartDevice: PW_STS      @+0x%04X = 0x%08lx\n",
                    WHL_PW_STS_OFFSET, pw_sts);

    } else {
        I2cCtrl_Log("StartDevice: PWRMBASE VA is NULL -> cannot dump PMC4/PW_* state\n");
    }

    /* 2. Dump LPSS private registers if BAR2 is mapped */
    if (fdoExt->LpssBar2 != NULL) {

        ULONG clk = 0, rst = 0, rst_sts = 0;
        ...
        __try {
            ...
            rst_sts = READ_REGISTER_ULONG(
                (PULONG)((PUCHAR)fdoExt->LpssBar2 + WHL_LPSS_RST_STS)
            );
        } __except(EXCEPTION_EXECUTE_HANDLER) {
            I2cCtrl_Log("StartDevice: EXCEPTION reading LPSS private registers\n");
        }

        I2cCtrl_Log("StartDevice: LPSS BAR2 VA = %p\n", fdoExt->LpssBar2);
        I2cCtrl_Log("StartDevice: LPSS CLK_CTL @+0x%04X = 0x%08lx\n",
                    WHL_LPSS_CLK_CTL, clk);
        I2cCtrl_Log("StartDevice: LPSS RST_CTL @+0x%04X = 0x%08lx\n",
                    WHL_LPSS_RST_CTL, rst);
        I2cCtrl_Log("StartDevice: LPSS RST_STS @+0x%04X = 0x%08lx\n",
                    WHL_LPSS_RST_STS, rst_sts);

    } else {
        I2cCtrl_Log("StartDevice: LPSS BAR2 is NULL -> cannot dump LPSS private state\n");
    }

    /* --- Existing WinDbg-style detailed diagnostics --- */
    {
        WCHAR wbuf[160];
        CHAR  abuf[160];
        UNICODE_STRING ustr;
    ...
    I2cCtrl_Log(
        "StartDevice: ENABLE FAILED (0x%08lx) - skipping synthetic ACPI PDO here\n",
        status
    );
    ...
    I2cCtrl_Log(
        "StartDevice: ReadyForChildren=TRUE, returning STATUS_SUCCESS so BusRelations can report PDOs\n"
    );
...
{
    //
    // 1. Reject missing or fabricated PWRMBASE immediately.
    //    On your ASUS X509FA + XP setup, PCI cannot expose the PMC device,
    //    so a zero or fallback value must abort the bring-up.
    //
    if (PwrmBase.QuadPart == 0) {
        I2cCtrl_Log("PWRMINFO: ERROR - No valid PWRMBASE from PCI/ACPI. "
                    "PMC is not exposed to this OS.\n");
        return STATUS_INVALID_PARAMETER;
    }

    //
    // 2. Align to 4KB boundary safely using a full 64-bit mask.
    //
    PwrmBase.QuadPart &= ~0xFFFULL;

    //
    // 3. Log using proper 64-bit formatting.
    //
    I2cCtrl_Log("PWRMINFO: Using PWRMBASE = 0x%016I64X (aligned)\n",
                PwrmBase.QuadPart);
...
// Reads PCI bus 0, device 31, function 2 (PMC), offset 0x48
//
NTSTATUS
    ...
    )
{
    ...
    I2cCtrl_Log("PWRMBASE-PCI: begin fetch (bus=0 dev=31 fn=2 offset=0x48)\n");

    RtlZeroMemory(&slot, sizeof(slot));
    slot.u.bits.DeviceNumber   = 31;  /* PCH PMC */
    slot.u.bits.FunctionNumber = 2;   /* <-- correct PMC function */
...
)
{
    ...
    /* ---------- CRITICAL FIX ---------- */
    pdo->Flags |= DO_POWER_PAGABLE;
    pdo->Flags |= DO_BUS_ENUMERATED_DEVICE;   // REQUIRED FOR PNP TO ENUMERATE CHILD
    pdo->Flags &= ~DO_DEVICE_INITIALIZING;
