...
/* XP fallback: derive LPSS BAR2 from GMHB() like ACPI does */
if (!haveBar2) {
    ...
    /* Read MHBR from PCI 0:0:0 at offset 0x48 (host bridge fabric base) */
    mhbr = I2cCtrl_ReadPciConfigDword(0, 0, 0, 0x48); /* MHBR */

    /* GMHB(): MH_B = MHBR << 0x0F */
    gmhb = mhbr << 15;

    /* MBAR = GMHB() + 0x5000, size 0x1000 */
    bar2.QuadPart = (ULONGLONG)gmhb + 0x5000;

    fdoExt->LpssBar2Phys   = bar2;
    ...
    if (fdoExt->LpssBar2 != NULL) {
        I2cCtrl_Log("StartDevice: LPSS BAR2 derived from GMHB and mapped\n");
    } else {
        I2cCtrl_Log("StartDevice: LPSS BAR2 GMHB-derived map FAILED\n");
    }
}
