 ...
 * ------------------------------------------------------------- */
{
    ...
    /* PWRMBASE comes from ACPI _CRS → MTFC (TPMC) */
    pwrmBase = fdoExt->PwrmBase;   /* you already store this after ACPI _CRS */

    if (pwrmBase.QuadPart != 0) {
        pmcStatus = I2cCtrl_EnablePmcPowerWellWhiskeyLake(pwrmBase);
    ...
    )
{
    ...
    /* Set all PMC4 bits (power well request) */
    val = *pmc4Reg;
    val |= WHL_PMC_PMC4_FULL_MASK;
    *pmc4Reg = val;
