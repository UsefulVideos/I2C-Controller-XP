...
/* -------------------------------------------------------------
 * Power wells + LPSS enable for Whiskey Lake / CNP‑LP
 * XP cannot use PMC IPC, so we hit PWRM power-well regs + LPSS BAR2.
 * ------------------------------------------------------------- */
...
if (fdoExt->PwrmBaseVa != NULL)
{
    ...
    I2cCtrl_Log("StartDevice: forcing WHL PW1/PW2 on via PWRM\n");

    /* -1. Enable CECE in PMC4 so direct PW_* writes take effect */
    {
        ULONG pmc4;
    ...
    /* 0b. Poll PW_STS until both wells are on */
    {
        ULONG timeout = 10000; /* ~10 ms */
        ...
        I2cCtrl_Log("StartDevice: WHL PW_STS = 0x%08lx\n", pw);
    }
}
else
{
    I2cCtrl_Log("StartDevice: no PWRM mapping -> cannot force PW1/PW2\n");
}

if (fdoExt->LpssBar2 != NULL)
{
    I2cCtrl_Log("StartDevice: enabling LPSS (WHL/CNL private registers)\n");

    /* 1. Enable all clocks: functional + APB + force-on */
    WRITE_REGISTER_ULONG(
    ...
    /* 3. Poll reset status */
    {
        ...
        I2cCtrl_Log("StartDevice: LPSS reset status = 0x%08lx\n", rst);
    }
}
else
{
    I2cCtrl_Log("StartDevice: LPSS BAR2 missing -> cannot enable LPSS\n");
}
