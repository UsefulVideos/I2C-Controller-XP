...
if (fdoExt == NULL) {
    I2cCtrl_Log("StartDevice: invalid fdoExt\n");
    return STATUS_INVALID_PARAMETER;
}

/*
 * Allow Irp == NULL when called from DetectTouchpadRedirect.
 * In this mode we skip IRP stack parsing but do NOT treat it as an error.
 */
if (Irp == NULL) {
    I2cCtrl_Log("StartDevice: redirector mode (Irp=NULL) -> skipping IRP stack parsing\n");
    isl = NULL;
    transList = NULL;
} else {
    isl = IoGetCurrentIrpStackLocation(Irp);
    transList = (isl != NULL)
        ? isl->Parameters.StartDevice.AllocatedResourcesTranslated
        : NULL;
}
