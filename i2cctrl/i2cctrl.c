    ...
    )
{
    ...
    /*
     * Walk DOWN the device stack to find the ACPI PDO.
     * On XP, ACPI PDO is the bottom-most device in the stack.
     */
    dev = fdoExt->Self;  /* our FDO */

    while (dev->AttachedDevice != NULL)
        dev = dev->AttachedDevice;

    /* Now dev is the PDO created by ACPI.SYS */
