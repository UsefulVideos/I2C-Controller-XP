...
{
    /* Align to 4K for consistency */
    PwrmBase.QuadPart &= 0xFFFFF000ULL;

    I2cCtrl_Log("PWRMINFO: reporting PWRMBASE = %08X%08X\n",
                PwrmBase.HighPart,
                PwrmBase.LowPart);

    if (PwrmBase.QuadPart == 0) {
        I2cCtrl_Log("PWRMINFO: PWRMBASE is zero (ACPI/PCI/policy did not provide one)\n");
    } else {
        I2cCtrl_Log("PWRMINFO: PWRMBASE appears valid\n");
    }
