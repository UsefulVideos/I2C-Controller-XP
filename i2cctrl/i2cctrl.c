    ...
    )
{
    ...
    if (devctx == NULL || devctx->MmioBase == NULL) {
        I2cCtrl_Log("LPSS2: PowerOff: invalid devctx or MMIO\n");
        return STATUS_NO_SUCH_DEVICE;
    }
    ...
    __try {
        ...
        /* ---------------------------------------------------------
         * 1) Mask interrupts immediately
         * --------------------------------------------------------- */
        I2cCtrl_Log("LPSS2: PowerOff: masking interrupts\n");
        WRITE_REGISTER_ULONG((PULONG)(base + devctx->RegIntrMask), 0xFFFFFFFF);

        /* ---------------------------------------------------------
         * 2) Clear any pending status
         * --------------------------------------------------------- */
        I2cCtrl_Log("LPSS2: PowerOff: clearing status\n");
        WRITE_REGISTER_ULONG((PULONG)(base + devctx->RegStatus), 0xFFFFFFFF);

        /* ---------------------------------------------------------
         * 3) Assert reset (LPSS2 reset = 1)
         * --------------------------------------------------------- */
        I2cCtrl_Log("LPSS2: PowerOff: asserting reset\n");
        WRITE_REGISTER_ULONG((PULONG)(base + devctx->RegReset), 0x00000001);

        /* ---------------------------------------------------------
         * 4) Disable clock gate
         * --------------------------------------------------------- */
        I2cCtrl_Log("LPSS2: PowerOff: disabling clock gate\n");
        WRITE_REGISTER_ULONG((PULONG)(base + devctx->RegClkCtl), 0x00000000);

        /* ---------------------------------------------------------
         * 5) Disable functional clock (CLKDIV = 0)
         * --------------------------------------------------------- */
        I2cCtrl_Log("LPSS2: PowerOff: disabling functional clock\n");
        WRITE_REGISTER_ULONG((PULONG)(base + devctx->RegClkDiv), 0x00000000);

        /* ---------------------------------------------------------
         * 6) Trigger CLKUPDATE to commit shutdown
         * --------------------------------------------------------- */
        I2cCtrl_Log("LPSS2: PowerOff: triggering CLKUPDATE\n");
        WRITE_REGISTER_ULONG((PULONG)(base + devctx->RegClkUpdate), 0x00000001);

        /* ---------------------------------------------------------
         * 7) Poll for CLKUPDATE completion
         * --------------------------------------------------------- */
        timeout = 1000;
        while (timeout--) {
