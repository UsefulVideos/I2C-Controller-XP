...
/* --- Enumerate ACPI children (XP‑safe, C89-compliant, HID‑I2C via HID table + ETPD) --- */
NTSTATUS
    ...
    )
{
    ...
    if (!outBuf) {
        I2cCtrl_Log("EnumerateAcpiChildren: out of memory for outBuf\n");
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if (!NT_SUCCESS(status)) {
        I2cCtrl_Log("EnumerateAcpiChildren: _HID eval failed (status=0x%08lx)\n", status);
        goto Done;
    }
    ...
    for (i = 0; i < outBuf->Count; i++) {
        ...
        if (arg->Type == ACPI_METHOD_ARGUMENT_STRING &&
            ...
            if (NT_SUCCESS(RtlAnsiStringToUnicodeString(&hidUni, &hidAnsi, TRUE))) {
                ...
                if (!staBuf) {
                    I2cCtrl_Log("EnumerateAcpiChildren: out of memory for staBuf\n");
                ...
                if (!NT_SUCCESS(I2cCtrl_AcpiEvalMethod(fdoExt, L"_STA",
                    ...
                    I2cCtrl_Log("EnumerateAcpiChildren: _STA eval failed or empty\n");
    ...
    if (NT_SUCCESS(dsmStatus)) {
        ...
        if (ParseHidDescriptorV10(dsmBuf, dsmLen, &parsed)) {
            ...
            RtlCopyMemory(&childDx->HidDesc,
                          dsmBuf,
                          (dsmLen < sizeof(HID_DESCRIPTOR)) ? dsmLen : sizeof(HID_DESCRIPTOR));
             ...
             */
            if (fdoExt->PnpId && wcsstr(fdoExt->PnpId, L"ELAN") != NULL) {
            ...
            } else if (fdoExt->PnpId && wcsstr(fdoExt->PnpId, L"GDIX") != NULL) {
            ...
            } else if (fdoExt->PnpId && wcsstr(fdoExt->PnpId, L"SYNA") != NULL) {
    ...
    )
{
    ...
    if (fdoExt == NULL) {
        I2cCtrl_Log("AcpiOpen: invalid fdoExt (NULL)\n");
        return STATUS_INVALID_PARAMETER;
    }
    ...
    I2cCtrl_Log("AcpiOpen: XP binding via PDO walk (HWID=%ws)\n",
                fdoExt->PnpId ? fdoExt->PnpId : L"<null>");

    /*
     * On XP, the ACPI PDO is at the bottom of the stack.
     * Walk down from our FDO's lower device until there is no AttachedDevice.
     */
    lower = fdoExt->LowerDevice;
    while (lower != NULL && lower->AttachedDevice != NULL) {
        lower = lower->AttachedDevice;
    }

    pdo = lower;

    if (pdo == NULL) {
        I2cCtrl_Log("AcpiOpen: no PDO found in stack\n");
        fdoExt->AcpiDeviceObject = NULL;
    ...
    /* For XP we just keep the PDO; no FILE_OBJECT, no extra referencing */
    fdoExt->AcpiDeviceObject = pdo;
    fdoExt->AcpiFileObject   = NULL;
    fdoExt->AcpiHandle       = NULL;
    fdoExt->AcpiBound        = TRUE;

    I2cCtrl_Log("AcpiOpen: ACPI bound (PDO=%p)\n", pdo);
 ...
 * - PASSIVE_LEVEL only
 * - Releases only the ACPI file object reference
 * - Does NOT close ACPI device objects or ACPI handles
    ...
    )
{
    ...
    I2cCtrl_Log("AcpiClose: closing ACPI binding for HWID=%ws\n",
                fdoExt->PnpId ? fdoExt->PnpId : L"<null>");
    ...
    /* XP driver: no ACPI namespace path to clear. Just log unbind. */
    ...
    )
{
    ...
    if (devctx == NULL || MethodName == NULL ||
        ...
        I2cCtrl_Log("AcpiEvalMethod: invalid parameters\n");
        return STATUS_INVALID_PARAMETER;
    }

    if (devctx->Removed) {
        I2cCtrl_Log("AcpiEvalMethod: device removed\n");
        return STATUS_DEVICE_REMOVED;
    }

    status = I2cCtrl_AcpiOpen(devctx);
    if (!NT_SUCCESS(status) || devctx->AcpiDeviceObject == NULL) {
        I2cCtrl_Log("AcpiEvalMethod: ACPI not connected\n");
        return STATUS_DEVICE_NOT_CONNECTED;
    }

    outLen = *OutBufferLen;
    if (outLen < sizeof(ACPI_EVAL_OUTPUT_BUFFER)) {
        I2cCtrl_Log("AcpiEvalMethod: output buffer too small\n");
        return STATUS_BUFFER_TOO_SMALL;
    }
    ...
    if (input == NULL) {
        I2cCtrl_Log("AcpiEvalMethod: allocation failed\n");
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if (!NT_SUCCESS(status)) {
        I2cCtrl_Log("AcpiEvalMethod: ACPI call failed (0x%08lx)\n", status);
        return status;
    }

    if (iosb.Information < sizeof(ACPI_EVAL_OUTPUT_BUFFER) ||
        iosb.Information > outLen) {
        I2cCtrl_Log("AcpiEvalMethod: buffer overflow\n");
...
/* XP fallback: hard‑code LPSS BAR2 if ACPI did not expose it */
if (!haveBar2) {

    fdoExt->LpssBar2Phys.QuadPart = 0xFE0F3000;   /* from Win11 */
    ...
    if (fdoExt->LpssBar2 != NULL) {
        I2cCtrl_Log("StartDevice: LPSS BAR2 hard‑coded and mapped\n");
    } else {
        I2cCtrl_Log("StartDevice: LPSS BAR2 hard‑coded map FAILED\n");
    }
}
...
if (!NT_SUCCESS(status)) {
    ...
    //
    // >>> NEW: force ACPI reset before enumeration <<<
    //
    I2cCtrl_AcpiClose(fdoExt);   /* ensure clean state */

    {
        NTSTATUS acpiStatus = I2cCtrl_AcpiOpen(fdoExt);
        if (!NT_SUCCESS(acpiStatus)) {
            I2cCtrl_Log("StartDevice: ACPI open failed (0x%08lx), skipping enumeration\n",
                        acpiStatus);
        } else {
            ULONG childCount = 0;
            NTSTATUS enumStatus =
            ...
            I2cCtrl_Log(
                "StartDevice: EnumerateAcpiChildren (after enable fail) → "
                "status=0x%08lx, children=%lu\n",
                enumStatus,
                childCount
            );
        }
    ...
    /*
     * NEW LOGIC:
     *  - If ACPI/ETPD already created children, we trust ACPI and skip
     *    legacy touchpad detection + synthetic PNP0C50 PDO.
     *  - If no children exist, we fall back to legacy HID-over-I2C
     *    detection and (optionally) create a generic PNP0C50 PDO.
     */

    if (!IsListEmpty(&fdoExt->ChildList)) {

        /* ACPI already gave us children (e.g. ETPD → ELAN1200/PNP0C50) */
        fdoExt->TouchpadPresent = TRUE; /* at least one HID child exists */
        I2cCtrl_Log("StartDevice: ChildList not empty, assuming ACPI touchpad present, skipping legacy detection/PNP0C50\n");

    } else {

        I2cCtrl_Log("StartDevice: ChildList empty, running legacy touchpad detection\n");

        /* Optional: probe for touchpad presence (legacy path) */
        {
            ...
            if (!NT_SUCCESS(status)) {
                ...
                I2cCtrl_Log(
                    ...
                    "StartDevice: touchpad present (legacy detection)\n" :
                    "StartDevice: touchpad not present (legacy detection)\n"
                );
            }
        }

        /* Create universal HID-over-I2C child PDO only if detection says present */
        if (fdoExt->TouchpadPresent) {

            WCHAR hidBuf[64];
            WCHAR uidBuf[32];

            I2cCtrl_Log("StartDevice: creating synthetic PNP0C50 PDO (legacy)\n");
            ...
            if (!NT_SUCCESS(status)) {
                ...
                I2cCtrl_Log("StartDevice: CreateChildPdo FAILED (legacy PNP0C50)\n");
                status = STATUS_SUCCESS; /* don't fail StartDevice just for this */

            } else {

                I2cCtrl_Log("StartDevice: CreateChildPdo SUCCESS (legacy PNP0C50), invalidating BusRelations\n");
                IoInvalidateDeviceRelations(fdoExt->PhysicalDevice, BusRelations);
            }
        } else {
            I2cCtrl_Log("StartDevice: no touchpad detected, skipping synthetic PNP0C50 PDO\n");
        }
    }
    ...
    )
{
    ...
    outSize = *outLen;
    if (outSize < sizeof(ACPI_EVAL_OUTPUT_BUFFER)) {
    ...
    {
        ACPI_METHOD_ARGUMENT UNALIGNED* arg = &out->Argument[0];

        if (arg->Type != ACPI_METHOD_ARGUMENT_BUFFER ||
            arg->DataLength == 0 ||
            arg->DataLength > outSize - sizeof(ACPI_EVAL_OUTPUT_BUFFER)) {
