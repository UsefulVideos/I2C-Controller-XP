...
/* -------------------------------------------------------------
 * 1. PWRMBASE (fixed)
 *    Firmware programs 0xFE0D0000 via DLLR() in the DSDT.
 *    ACPI \PWRM does not exist on this platform.
 * ------------------------------------------------------------- */

if (fdoExt->PwrmBase.QuadPart == 0)
{
    fdoExt->PwrmBase.LowPart  = 0xFE0D0000UL;   /* actual PWRMBASE */
    fdoExt->PwrmBase.HighPart = 0;

    I2cCtrl_Log("StartDevice: PWRMBASE (fixed) = %08X%08X\n",
...
/* PWRMBASE policy:
 *  - always use the firmware‑programmed base (0xFE0D0000)
 *    discovered via ACPI DLLR() and PMC BAR programming
 */
Dx->PolicyPwrmBase.LowPart  = 0xFE0D0000U;
...
{
    /* Ignore caller's value on XP, but allow universal detection */
    /* -------------------------------------------------------------
     * 1. Universal detection (PCI/ACPI/etc.)
     * ------------------------------------------------------------- */
        ...
        PwrmBase.QuadPart != 0xFFFFFFFFULL)
    {
        ...
        I2cCtrl_Log("PWRMINFO: Universal PWRMBASE = 0x%016I64X\n",
                    PwrmBase.QuadPart);
    ...
    /* -------------------------------------------------------------
     * 2. Universal detection failed → fallback to firmware value.
     *    DSDT programs PWRMBASE = 0xFE0D0000 via DLLR().
     * ------------------------------------------------------------- */
    PwrmBase.QuadPart = 0xFE0D0000ULL;

    I2cCtrl_Log("PWRMINFO: Fallback PWRMBASE = 0x%016I64X\n",
                PwrmBase.QuadPart);
