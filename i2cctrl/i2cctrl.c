...
if (!haveBar2)
{
    ...
    if (fdoExt->LpssBar2 != NULL) {
        haveBar2 = TRUE;   /* ensure consistency */
...
/* -------------------------------------------------------------
 * Minimal LPSS force‑enable (Whiskey Lake / CNP‑LP)
 * XP cannot use PMC IPC, so we manually ungate clock + clear reset.
 * ------------------------------------------------------------- */
if (fdoExt->LpssBar2 != NULL)
{
    I2cCtrl_Log("StartDevice: forcing LPSS clock + reset (XP fallback)\n");

    /* 1. Enable functional clock */
    WRITE_REGISTER_ULONG(
        (PULONG)(fdoExt->LpssBar2 + LPSS_CLK_CTL),
        1
    );

    /* 2. Commit clock change */
    WRITE_REGISTER_ULONG(
        (PULONG)(fdoExt->LpssBar2 + LPSS_CLK_UPDATE),
        1
    );

    /* 3. Deassert reset */
    WRITE_REGISTER_ULONG(
        (PULONG)(fdoExt->LpssBar2 + LPSS_RST_CTL),
        0
    );
} else {
    I2cCtrl_Log("StartDevice: LPSS BAR2 missing -> cannot force-enable LPSS\n");
}
    ...
    )
{
    ...
    I2cCtrl_Log("PMC: enabling power wells at PWRMBASE=%08X%08X\n",
                PwrmBase.HighPart, PwrmBase.LowPart);

    /* Map enough of the PWRM region to reach 0x5C8 */
    pwrmVa = (PUCHAR)MmMapIoSpace(PwrmBase, 0x1000, MmNonCached);
    if (pwrmVa == NULL) {
        I2cCtrl_Log("PMC: MmMapIoSpace FAILED\n");
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    I2cCtrl_Log("PMC: mapped PWRM region @ %p\n", pwrmVa);

    /* 1. Force PW1 + PW2 on */
    I2cCtrl_Log("PMC: writing PW_FORCE_ON (0x5C8) = 0x%02X\n", WHL_PW_MASK);
    WRITE_REGISTER_ULONG(
        (PULONG)(pwrmVa + WHL_PW_FORCE_ON_OFFSET),
        WHL_PW_MASK
    );

    /* 2. Request PW1 + PW2 */
    I2cCtrl_Log("PMC: writing PW_CTL (0x5C0) = 0x%02X\n", WHL_PW_MASK);
    WRITE_REGISTER_ULONG(
        (PULONG)(pwrmVa + WHL_PW_CTL_OFFSET),
        WHL_PW_MASK
    );

    /* 3. Poll PW_STS until both wells are on */
    I2cCtrl_Log("PMC: polling PW_STS (0x5C4) for PW1/PW2 ready...\n");

    for (;;) {

        val = READ_REGISTER_ULONG((PULONG)(pwrmVa + WHL_PW_STS_OFFSET));

        if ((val & WHL_PW_MASK) == WHL_PW_MASK) {
            I2cCtrl_Log("PMC: PW_STS ready (0x%08lx)\n", val);
            status = STATUS_SUCCESS;
            break;
        }

        if (waitedUs >= 500000) { /* 500 ms timeout */
            I2cCtrl_Log("PMC: TIMEOUT waiting for PW_STS (last=0x%08lx)\n", val);
            status = STATUS_IO_TIMEOUT;
            break;
        }
    ...
    MmUnmapIoSpace(pwrmVa, 0x1000);

    if (NT_SUCCESS(status)) {
        I2cCtrl_Log("PMC: power wells enabled successfully\n");
    } else {
        I2cCtrl_Log("PMC: power well enable FAILED (0x%08lx)\n", status);
    }
