...
/* -------------------------------------------------------------
 * LPSS enable for Whiskey Lake / CNP‑LP
 * XP cannot use PMC IPC, so we use LPSS private registers in BAR2.
 * ------------------------------------------------------------- */
...
if (fdoExt->LpssBar2 != NULL)
{
    ...
    WRITE_REGISTER_ULONG(
        (PULONG)(fdoExt->LpssBar2 + WHL_LPSS_CLK_CTL),
        0x7
    );

    /* 2. Deassert reset */
    WRITE_REGISTER_ULONG(
        (PULONG)(fdoExt->LpssBar2 + WHL_LPSS_RST_CTL),
        0x0
    );

    /* 3. Optional: poll reset status */
    {
        ULONG rst;
        ...
        do {
            rst = READ_REGISTER_ULONG(
                (PULONG)(fdoExt->LpssBar2 + WHL_LPSS_RST_STS)
            );
            if (rst & 0x1)
                break;
