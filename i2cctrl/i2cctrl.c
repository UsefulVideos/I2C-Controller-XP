...
{
    NTSTATUS        status;
    PDEVICE_OBJECT  acpiDev  = NULL;
    PFILE_OBJECT    acpiFile = NULL;
    ...
    //
    // Build a real UNICODE_STRING for XP compatibility.
    //
    ...
    fdoExt->AcpiDeviceObject = acpiDev;
    fdoExt->AcpiFileObject   = acpiFile;
    fdoExt->AcpiHandle       = NULL;
    fdoExt->AcpiBound        = TRUE;

    I2cCtrl_Log("AcpiOpen: bound to ACPI device %p\n", acpiDev);
    ...
    )
{
    ...
    //
    // XP/2003 ACPI binding:
    //  - IoGetDeviceObjectPointer returns a FILE_OBJECT only if the
    //    ACPI namespace object is opened as a file.
    //  - For \Device\ACPI, XP returns a FILE_OBJECT that must be dereferenced.
    //
...
/* -------------------------------------------------------------
 * PWRMBASE selection (XP-safe, WHL/CNL-correct)
 * Order:
 *   1. Caller/PCI/ACPI (already attempted earlier)
 *   2. Registry override (PolicyPwrmBase)
 *   3. Firmware fallback = 0xFE0D0000 (DLLR)
 * ------------------------------------------------------------- */
...
/* 2. Firmware fallback (WHL/CNL DLLR = FE0D0000) */
if (fdoExt->PwrmBase.QuadPart == 0)
{
    fdoExt->PwrmBase.LowPart  = 0xFE0D0000UL;
    fdoExt->PwrmBase.HighPart = 0;

    I2cCtrl_Log("StartDevice: PWRMBASE (fallback) = %08X%08X\n",
                fdoExt->PwrmBase.HighPart,
                fdoExt->PwrmBase.LowPart);
}
...
if (fdoExt->PwrmBaseVa == NULL)
{
    fdoExt->PwrmBaseVa = MmMapIoSpace(
        fdoExt->PwrmBase,
        0x2000,        /* covers PMC4, PW_FORCE_ON, PW_STS */
        MmNonCached
    );
...
if (!NT_SUCCESS(status)) {
    ...
    I2cCtrl_Log("StartDevice: ENABLE FAILED (0x%08lx) -> collecting generic debug\n", status);

    /* 1. Dump PWRM window if mapped (generic labels) */
    if (fdoExt->PwrmBaseVa != NULL) {

        ULONG r0 = 0, r1 = 0, r2 = 0;

        __try {
            r0 = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + 0x0));
            r1 = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + 0x4));
            r2 = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->PwrmBaseVa + 0x8));
        } __except(EXCEPTION_EXECUTE_HANDLER) {
            I2cCtrl_Log("StartDevice: EXCEPTION reading generic PWRM window\n");
        }

        I2cCtrl_Log("StartDevice: PWRMBASE VA = %p\n", fdoExt->PwrmBaseVa);
        I2cCtrl_Log("StartDevice: PWRM[0x00] = 0x%08lx\n", r0);
        I2cCtrl_Log("StartDevice: PWRM[0x04] = 0x%08lx\n", r1);
        I2cCtrl_Log("StartDevice: PWRM[0x08] = 0x%08lx\n", r2);

    } else {
        I2cCtrl_Log("StartDevice: PWRMBASE VA is NULL -> no PWRM dump\n");
    }

    /* 2. Dump LPSS/private window if BAR2 is mapped (generic labels) */
    if (fdoExt->LpssBar2 != NULL) {

        ULONG r0 = 0, r1 = 0, r2 = 0;

        __try {
            r0 = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + 0x0));
            r1 = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + 0x4));
            r2 = READ_REGISTER_ULONG((PULONG)((PUCHAR)fdoExt->LpssBar2 + 0x8));
        } __except(EXCEPTION_EXECUTE_HANDLER) {
            I2cCtrl_Log("StartDevice: EXCEPTION reading generic LPSS/private window\n");
        }

        I2cCtrl_Log("StartDevice: LPSS/PRIV VA = %p\n", fdoExt->LpssBar2);
        I2cCtrl_Log("StartDevice: PRIV[0x00] = 0x%08lx\n", r0);
        I2cCtrl_Log("StartDevice: PRIV[0x04] = 0x%08lx\n", r1);
        I2cCtrl_Log("StartDevice: PRIV[0x08] = 0x%08lx\n", r2);

    } else {
        I2cCtrl_Log("StartDevice: LPSS/PRIV VA is NULL -> no private dump\n");
    }
    ...
    /* 4. Clean up hardware resources (universal) */
    if (fdoExt->InterruptObject != NULL) {
    ...
    if (fdoExt->LpssBar2 != NULL) {
        I2cCtrl_Log("StartDevice: unmapping LPSS/PRIV at %p (len=%lu)\n",
                    fdoExt->LpssBar2,
                    fdoExt->LpssBar2Length);
    ...
    if (fdoExt->PwrmBaseVa != NULL) {
        ...
        MmUnmapIoSpace(fdoExt->PwrmBaseVa, 0x2000);
