...
for (outer = 0U; outer < transList->Count; outer++) {
    ...
    for (inner = 0U; inner < prl->Count; inner++) {
        ...
        if (desc->Type == CmResourceTypeMemory) {
            ...
            /* DO NOT look for PWRMBASE here - it is NOT in this _CRS */
...
//
// Whiskey Lake / CNP-LP: fetch PMC PWRMBASE from ACPI (PWRM)
// Only do this once per controller.
//
if (fdoExt->PwrmBase.QuadPart == 0 && fdoExt->AcpiDeviceObject != NULL) {
    ...
    if (outBuf != NULL) {
        ...
        //
        // NOTE: PWRM is a global integer, not a method.
        // We still use the same helper to evaluate it.
        //
        acpiStatus = I2cCtrl_AcpiEvalMethod(
                         fdoExt->AcpiDeviceObject,
                         NULL,          // no ACPI handle
                         "\\PWRM",      // global ACPI object name
                         outBuf,
                         outLen
                     );
        ...
        if (NT_SUCCESS(acpiStatus) && outBuf->Count >= 1) {
                ...
                arg->DataLength >= sizeof(ULONG))
            {
                ...
                if (pwrmVal != 0 && pwrmVal != 0xFFFFFFFFUL) {
                    ...
                    fdoExt->PwrmBase.LowPart  = pwrmVal;
                    fdoExt->PwrmBase.HighPart = 0;

                    I2cCtrl_Log("StartDevice: PWRMBASE (PWRM) = %08X%08X\n",
                                fdoExt->PwrmBase.HighPart,
                                fdoExt->PwrmBase.LowPart);
                } else {
                    I2cCtrl_Log("StartDevice: PWRM returned invalid value (0x%08lx)\n",
                                pwrmVal);
                }
            } else {
                I2cCtrl_Log("StartDevice: PWRM ACPI arg[0] not INTEGER or too small\n");
            }
        } else {
        ...
        I2cCtrl_Log("StartDevice: failed to allocate ACPI output buffer for PWRM\n");
    }
}

/* NEW: PCI fallback if ACPI \PWRM did not provide a base */
if (fdoExt->PwrmBase.QuadPart == 0) {
    PHYSICAL_ADDRESS pwrm;
    NTSTATUS         pciStatus;

    pciStatus = I2cCtrl_FetchPwrmBaseFromPciWhl(&pwrm);
    if (NT_SUCCESS(pciStatus)) {
        fdoExt->PwrmBase = pwrm;
...
/* -------------------------------------------------------------
 * Whiskey Lake / CNP-LP: PMC power-well enable (PCI fallback)
 * ------------------------------------------------------------- */
{
    NTSTATUS pmcStatus;

    /* If ACPI \PWRM failed, try PCI PWRMBASE */
    if (fdoExt->PwrmBase.QuadPart == 0) {
    ...
    /* Now attempt PMC power-well enable */
    if (fdoExt->PwrmBase.QuadPart != 0) {

        pmcStatus = I2cCtrl_EnablePmcPowerWellWhiskeyLake(
                        fdoExt->PwrmBase
                    );
        ...
        if (NT_SUCCESS(pmcStatus)) {
            ...
            I2cCtrl_Log("StartDevice: PMC power-well enable FAILED (0x%08lx)\n",
                        pmcStatus);
        }

    } else {
        I2cCtrl_Log("StartDevice: no valid PWRMBASE (ACPI+PCI), skipping PMC power-well\n");
    }
}
...
if (!NT_SUCCESS(status)) {
...

    {
        ...
        if (!NT_SUCCESS(acpiStatus)) {
            ...
            enumStatus =
                I2cCtrl_EnumerateAcpiNamespace(
                    fdoExt->AcpiDeviceObject,   /* ACPI PDO */
                    NULL,                       /* start at root */
                    0,                          /* depth */
                    fdoExt                      /* controller context */
                );
