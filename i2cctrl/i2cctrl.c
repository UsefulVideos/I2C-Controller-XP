    ...
    )
{
    ...
    /* -------------------------------------------------------------
     * Initialize ISR/DPC ring-buffer logger (nonpaged)
     * ------------------------------------------------------------- */
    KeInitializeSpinLock(&I2cCtrl_IsrLogLock);
    I2cCtrl_IsrLogIndex = 0;
    RtlZeroMemory(I2cCtrl_IsrLogBuffer, I2CCTRL_ISRLOG_SIZE);
    ...
    )
{
    ...
    //
    // Always: append to ISR ring buffer (DIRQL-safe)
    //
    KeAcquireSpinLock(&I2cCtrl_IsrLogLock, &oldIrql);

    if (len >= I2CCTRL_ISRLOG_SIZE) {
        len = I2CCTRL_ISRLOG_SIZE - 1;
    }

    if (I2cCtrl_IsrLogIndex + len >= I2CCTRL_ISRLOG_SIZE) {
        I2cCtrl_IsrLogIndex = 0; /* wrap */
    }

    RtlCopyMemory(&I2cCtrl_IsrLogBuffer[I2cCtrl_IsrLogIndex], buffer, len);
    I2cCtrl_IsrLogIndex += len;

    KeReleaseSpinLock(&I2cCtrl_IsrLogLock, oldIrql);

    //
    // ETW/WPP mirror (IRQL-safe)
    //
    TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_FLAG_ISR, "%s", buffer);

    //
    // If we are at PASSIVE_LEVEL, call the full logger
    //
    irql = KeGetCurrentIrql();
    if (irql == PASSIVE_LEVEL) {
        I2cCtrl_Log("%s", buffer);
    }
}
    ...
    )
{
    ...
    for (;;) {
        ...
        if (fdoExt == NULL) {
            break;  /* or sleep+continue, depending on how you want to stop */
        }

        req.Pin = pdoExt->GpioPin;   /* you can also use HidGpioPin if you prefer */
        ...
        if (!NT_SUCCESS(status)) {
            /* TODO: add stop/termination check later */
            continue;
        }

        /* 1) Re-read HID descriptor (safe, non-destructive) */
        (void)I2cCtrl_ReadHidDescriptor(
                  fdoExt,
                  pdoExt,
                  &hidDesc
              );

        /* 2) Re-read report descriptor (safe, non-destructive) */
        if (pdoExt->HidReportDescLen > 0 &&
            pdoExt->HidReportDescLen <= sizeof(tempBuf))
        {
            (void)I2cCtrl_ReadReportDescriptor(
                      fdoExt,
                      pdoExt,
                      tempBuf,
        ...
        /* 3) Read a HID register (safe, generic) */
        (void)I2cCtrl_ReadHidRegister(
                  fdoExt,
