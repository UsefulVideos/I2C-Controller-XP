...
/*
 * NEW LOGIC:
 *  - If ACPI already created children, trust ACPI.
 *  - If ACPI created nothing, ALWAYS create synthetic I2CCTRL\TOUCHPAD PDO.
 */

if (!IsListEmpty(&fdoExt->ChildList)) {

    /* ACPI already gave us children (ETPD/ELAN/PNP0C50/etc.) */
    fdoExt->TouchpadPresent = TRUE;
    I2cCtrl_Log("StartDevice: ACPI child present, skipping synthetic ACPI PDO\n");

} else {

    I2cCtrl_Log("StartDevice: No ACPI children -> creating synthetic I2CCTRL\\TOUCHPAD PDO\n");

    status = I2cCtrl_CreateTouchpadPdo(fdoExt->Self, fdoExt);

    if (NT_SUCCESS(status)) {
        fdoExt->TouchpadPresent = TRUE;
        I2cCtrl_Log("StartDevice: synthetic I2CCTRL\\TOUCHPAD PDO created\n");
        IoInvalidateDeviceRelations(fdoExt->PhysicalDevice, BusRelations);
    } else {
        fdoExt->TouchpadPresent = FALSE;
        I2cCtrl_Log("StartDevice: FAILED to create synthetic I2CCTRL\\TOUCHPAD PDO (0x%08lx)\n", status);
    }
}
