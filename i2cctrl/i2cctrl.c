...
//
// Whiskey Lake / CNP-LP: fetch PMC PWRMBASE from ACPI (TPMC)
...
if (fdoExt->PwrmBase.QuadPart == 0 && fdoExt->AcpiDeviceObject != NULL) {
    ...
    if (outBuf != NULL) {
        ...
        //
        // Correct signature:
        // I2cCtrl_AcpiEvalMethod(Pdo, Handle, MethodName, OutBuf, OutBufLen)
        //
        acpiStatus = I2cCtrl_AcpiEvalMethod(
                         fdoExt->AcpiDeviceObject,
                         NULL,          // no ACPI handle
                         "TPMC",        // ACPI method name
                         outBuf,        // *** NOT &outBuf ***
                         outLen
                     );
        ...
        if (NT_SUCCESS(acpiStatus) && outBuf->Count >= 1) {
            ...
            //
            // Arguments are packed in Data[] as ACPI_METHOD_ARGUMENTs
            //
            ...
            {
                ULONG64 tpmcVal = *(ULONG64 *)arg->Data;

                if (tpmcVal != 0 && tpmcVal != 0xFFFFFFFFULL) {

                    fdoExt->PwrmBase.QuadPart = tpmcVal;

                    I2cCtrl_Log("StartDevice: PWRMBASE (TPMC) = %08X%08X\n",
                                fdoExt->PwrmBase.HighPart,
                                fdoExt->PwrmBase.LowPart);
                } else {
                    I2cCtrl_Log("StartDevice: TPMC returned invalid value (0x%I64x)\n",
                                tpmcVal);
                }
            } else {
                I2cCtrl_Log("StartDevice: TPMC ACPI arg[0] not INTEGER or too small\n");
            }
        } else {
            I2cCtrl_Log("StartDevice: TPMC ACPI eval failed (0x%08lx)\n",
                        acpiStatus);
        }

        ExFreePool(outBuf);
    } else {
        I2cCtrl_Log("StartDevice: failed to allocate ACPI output buffer for TPMC\n");
    }
}


/* -------------------------------------------------------------
 * NEW: Whiskey Lake / CNP-LP PMC power-well enable
 * ------------------------------------------------------------- */
{
    NTSTATUS pmcStatus;
    ...
    if (fdoExt->PwrmBase.QuadPart != 0) {
        ...
        I2cCtrl_Log("StartDevice: no PWRMBASE available, skipping PMC power-well\n");
    }
}
    ...
    )
{
    ...
    /* Request all PMC4 power wells (bits 0–30) */
    val = *pmc4Reg;
