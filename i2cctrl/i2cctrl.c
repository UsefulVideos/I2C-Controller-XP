...
if (!NT_SUCCESS(status)) {
...

    {
        ...
        I2cCtrl_Log("StartDevice: ENABLE FAILED (0x%08lx) - forcing synthetic touchpad PDO\n", status);

        pdoStatus = I2cCtrl_CreateTouchpadPdo(fdoExt->Self, fdoExt);

        if (NT_SUCCESS(pdoStatus)) {
            I2cCtrl_Log("StartDevice: synthetic touchpad PDO created\n");
            fdoExt->TouchpadPresent = TRUE;
            IoInvalidateDeviceRelations(fdoExt->PhysicalDevice, BusRelations);
        } else {
            I2cCtrl_Log("StartDevice: FAILED to create synthetic touchpad PDO (0x%08lx)\n", pdoStatus);
        }
    }
...
)
{
    ...
    RtlZeroMemory(pdoExt, sizeof(I2CCTRL_PDO));
    ...
    RtlInitUnicodeString(&pdoExt->HardwareId,  L"I2CCTRL\\TOUCHPAD");
    RtlInitUnicodeString(&pdoExt->CompatibleId, L"HID_DEVICE");
    ...
    RtlInitUnicodeString(&pdoExt->InstanceId, L"1");
    RtlInitUnicodeString(&pdoExt->ClassName, L"Mouse");

    pdoExt->ClassGuid = GUID_DEVCLASS_MOUSE;

    RtlInitUnicodeString(&pdoExt->DeviceDesc,
                         L"Touchpad-like pointing device");
