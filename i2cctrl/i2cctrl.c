    ...
    )
{
    ...
    /*
     * XP/2003 driver: no ACPI namespace path strings are stored.
     * ACPI binding is handled entirely by I2cCtrl_AcpiOpen().
     */
    I2cCtrl_Log("AcpiOpen: using default ACPI binding for controller HWID=%ws\n",
                fdoExt->PnpId ? fdoExt->PnpId : L"<null>");

    /* Open \Device\ACPI for IOCTLs */
    ...
    status = IoGetDeviceObjectPointer(&acpiName,
                                      ...
                                      &devObj);
    if (!NT_SUCCESS(status)) {
        I2cCtrl_Log("AcpiOpen: IoGetDeviceObjectPointer failed (0x%08lx)\n",
                    status);
        fdoExt->AcpiDeviceObject = NULL;
        fdoExt->AcpiFileObject   = NULL;
    ...
    I2cCtrl_Log("AcpiOpen: ACPI bound (DevObj=%p, FileObj=%p)\n",
                devObj, fileObj);
